...
sys.path.append(os.path.join(root, r"src", r"python_routing_v02"))
...
def compute_mc_reach_up2down(
    ...
    while True:
        ...
        routelink_data = nhd_io.read(network_data["geo_file_path"])
        routelink_data = routelink_data[list(cols.values())]
        routelink_data = routelink_data.set_index(cols["key"])
        ...
        if "mask_file_path" in network_data:
            ...
            routelink_data = routelink_data.filter(data_mask.iloc[:, network_data["mask_key"]], axis=0)

        routelink_data = routelink_data.sort_index()
        routelink_data = nhd_io.replace_downstreams(routelink_data, cols["downstream"], 0)

        if args.ql:
            qlats = nhd_io.read_qlat(args.ql)
        else:
            qlats = constant_qlats(routelink_data, nts, 10.0)

        connections = nhd_network.extract_connections(routelink_data, cols["downstream"])
        wbodies = nhd_network.extract_waterbodies(
            routelink_data, cols["waterbody"], network_data["waterbody_null_code"]
        )
        ...
        routelink_data["dt"] = 300.0
        routelink_data = routelink_data.rename(columns=nnu.reverse_dict(cols))
        routelink_data = routelink_data.astype("float32")

        # datasub = routelink_data[['dt', 'bw', 'tw', 'twcc', 'dx', 'n', 'ncc', 'cs', 's0']]
