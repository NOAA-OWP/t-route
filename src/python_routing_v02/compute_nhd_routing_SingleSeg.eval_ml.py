...
def compute_mc_reach_up2down(
    ...
    while True:
        if ASSUME_SHORT_TS: quc_tmp = qup_tmp
...
def singlesegment(
    ...
    # call ML routine
    input_arr = []
    input_arr.append([dt, qup, quc, qdp, qlat, dx, bw, tw, twcc
...
def main(parallelcompute = True, num_cores = 8, sort = 'natural'):
    ...
    if sort is 'natural':
        ...
        sorted_networks = list(networks.items())
        sorted_networks.sort(reverse = True, key = lambda networks_tuple: networks_tuple[1]['total_segment_count'])
    elif sort is 'sort':
        sorted_networks = list(networks.items())
        sorted_networks.sort(reverse = False, key = lambda networks_tuple: networks_tuple[1]['total_segment_count'])

    if showtiming: compute_start_time = time.time()
    ...
    if not parallelcompute:
        ...
        # for terminal_segment, network in sorted(list(networks.items())
            ...
            if showtiming: n_start_time = time.time()
            compute_network(
            ...
            if terminal_segment == 22811611:
                ...
                if showtiming: print("... in %s seconds." % (time.time() - n_start_time))
        ...
        nslist = ([terminal_segment
                  ...
                  #for terminal_segment, network in sorted(list(networks.items())
                                                  #, reverse = False
                                                  #, key = lambda item: item[1]['total_segment_count']))
        if showtiming: p_start_time = time.time()
        with multiprocessing.Pool(num_cores) as pool:
            results = pool.starmap(compute_network, nslist)
        if showtiming: print("... in %s seconds." % (time.time() - p_start_time))

    if verbose: print('ordered reach computation complete')
    if showtiming: print("... in %s seconds." % (time.time() - compute_start_time))
