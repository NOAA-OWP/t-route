...
def compute_reach(
    reach, qup, quc, data, flowdepthvel, assume_short_ts=False,
):
...
def compute_mc_reach_up2down_TS(
    nts=0,
    reaches=None,
    connections=None,
    data=None,
    ...
    for ts in range(nts):
        for i, reach in enumerate(reaches, 1):
            ...
            print(f"timestep={ts} \t reach={i}".ljust(50), end="\r")
    return
...
def compute_mc_reach_up2down(
    ...
    compute_reach(
        pluck(0, reach), qup, quc, data, flowdepthvel, assume_short_ts=assume_short_ts
    )
...
def translate_network_to_index(connections, index):
    ...
    for n, children in connections.items():
        rv[n] = {
            "index": index.get_loc(n),
            "children": tuple(zip(np.searchsorted(index, children), children)),
        }
    return rv
...
def main():
    ...
    for tw, net in subnets.items():
        reaches = nhd_network.dfs_decomposition(nhd_network.reverse_network(net))
        subreaches[tw] = translate_reach_to_index(reaches, data.index)
    ...
    if not parallelcompute:
        ...
        for twi, (tw, reaches) in enumerate(subreaches.items(), 1):
            print(f"network={twi} (tw_seg={tw})\t")
            compute_mc_reach_up2down_TS(
                nts=nts,
                reaches=reaches,
            ...
            if showtiming:
                print(
                    f"... in {time.time()-start_time} seconds ({twi}/{len(subreaches)})"
                )
        ...
        nslist = (
            [
                ...
                reaches,
                subnets[tw],
                data_values,  # TODO: This should probably be global...
                flowdepthvel,
            ]
            for twi, (tw, reaches) in enumerate(subreaches.items(), 1)
        )
        with multiprocessing.Pool() as pool:
            results = pool.starmap(compute_mc_reach_up2down_TS, nslist)
