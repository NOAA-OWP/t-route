...
contains
    ...
    subroutine diffnw(timestep_ar_g, nts_ql_g, nts_ub_g, nts_db_g, ntss_ev_g, nts_qtrib_g, &
        ...
        ! create dx array from dx_ar_g and determinint minimum dx.
        dx = 0.
        minDx = 1e10
        !do j = 1,nlinks
        ...
        ! Determinining leftBank and rightBank locations
...
!        ============= retaining commented code for future downstream boundary development ===============
...
!            stop
    ...
    subroutine mesh_diffusive_backward(dtini_given, t0, t, tfin, saveInterval,j, leftBank, rightBank)
        ...
        double precision :: skkkTable_1(nel), dKdATable_1(nel), pereTable_1(nel),currentSquaredDepth_1(nel)                                                     
            ...
            if (newMassBalance .eq. 1) then
                ...
                if (i .gt. 1) then
                    ...
                    if ( (routingNotChanged(i-1,j) .lt. minNotSwitchRouting2) .and. (currentRoutingNormal(i-1,j) .lt. 3) ) then
                        ...
                        else if ( (dimensionless_D(i-1,j) .ge. DD_ulm) .and. (dimensionless_D(i-1,j) .lt. DD_llm) ) then
