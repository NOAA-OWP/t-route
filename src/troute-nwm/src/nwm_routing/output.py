...
class NWMOutputGenerator:
    def __init__(
        ...
        self.parity_parameters = parity_parameters
        self.restart_parameters = restart_parameters
        self.parity_set = parity_set
        self.qts_subdivisions = qts_subdivisions
        self.return_courant = return_courant
        self.cpu_pool = cpu_pool
        self.waterbodies_df = waterbodies_df
        self.waterbody_types_df = waterbody_types_df
        self.duplicate_ids_df = duplicate_ids_df
        self.data_assimilation_parameters = data_assimilation_parameters
        self.lastobs_df = lastobs_df
        self.link_gage_df = link_gage_df
        self.link_lake_crosswalk = link_lake_crosswalk
        self.nexus_dict = nexus_dict
        self.poi_crosswalk = poi_crosswalk
        self.logFileName = logFileName

        self.dt = self.run.get("dt")
        self.nts = self.run.get("nts")
        self.t0 = self.run.get("t0")
        
        self.setup_parity_parameters()
        self.handle_output()
        self.perform_parity_check()  

    def _reindex_lake_to_link_id(self,target_df, crosswalk):
        '''
        ...
        # set the target_df index
        target_df.set_index(idxs, inplace = True)

        return target_df


    def _parquet_output_format_converter(self,df, start_datetime, dt, configuration, prefix_ids):
        '''
        ...
        df['location_id'] = prefix_ids + '-' + df['location_id'].astype('string')
        
        # Flatten the dataframe using NumPy to speed it up
        location_ids = df['location_id'].repeat(df.shape[1] - 1)
        times = np.tile(df.columns[1:], len(df))
        values = df.values[:, 1:].flatten()
        
        # Extract timesteps and variables
        timestep, variable = zip(*times)
        timestep = np.array(timestep, dtype=int)
        variable = np.array(variable)
        
        # Create value_time using vectorized operations
        value_time = start_datetime + pd.to_timedelta(timestep * dt, unit='s')
        
        # Map variable names and units
        variable_to_name_map = {"q": "streamflow", "d": "depth", "v": "velocity"}
        variable_to_units_map = {"streamflow": "m3/s", "velocity": "m/s", "depth": "m"}
        variable_names = np.vectorize(variable_to_name_map.get)(variable)
        units = np.vectorize(variable_to_units_map.get)(variable_names)
    ...
    def setup_parity_parameters(self):
        if self.parity_parameters:
            self.parity_set["parity_check_waterbody_file"] = self.parity_set.get(
                "parity_check_waterbody_file", self.parity_parameters.get("parity_check_waterbody_file", None)
            )
            self.parity_set["parity_check_file"] = self.parity_set.get(
                "parity_check_file", self.parity_parameters.get("parity_check_file", None)
            )
            self.parity_set["parity_check_input_folder"] = self.parity_set.get(
                "parity_check_input_folder", self.parity_parameters.get("parity_check_input_folder", None)
            )
            self.parity_set["parity_check_file_index_col"] = self.parity_set.get(
                "parity_check_file_index_col", self.parity_parameters.get("parity_check_file_index_col", None)
            )
            self.parity_set["parity_check_file_value_col"] = self.parity_set.get(
                "parity_check_file_value_col", self.parity_parameters.get("parity_check_file_value_col", None)
            )
            self.parity_set["parity_check_compare_node"] = self.parity_set.get(
                "parity_check_compare_node", self.parity_parameters.get("parity_check_compare_node", None)
            )
    ...
    def handle_output(self):
        ...
        csv_output = self.output_parameters.get("csv_output", None)
        csv_output_folder = None
        parquet_output = self.output_parameters.get("parquet_output", None)
        parquet_output_folder = None
        rsrto = self.output_parameters.get("hydro_rst_output", None)
        chrto = self.output_parameters.get("chrtout_output", None)
        test = self.output_parameters.get("test_output", None)
        chano = self.output_parameters.get("chanobs_output", None)
        wbdyo = self.output_parameters.get("lakeout_output", None)
        stream_output = self.output_parameters.get("stream_output", None)
        lastobso = self.output_parameters.get("lastobs_output", None)

        if csv_output:
            csv_output_folder = self.output_parameters["csv_output"].get("csv_output_folder", None)
            csv_output_segments = csv_output.get("csv_output_segments", None)

        if parquet_output:
            parquet_output_folder = self.output_parameters["parquet_output"].get("parquet_output_folder", None)
        ...
        if csv_output_folder or parquet_output_folder or rsrto or chrto or chano or test or wbdyo or stream_output:
            start = time.time()
            qvd_columns = pd.MultiIndex.from_product([range(self.nts), ["q", "v", "d"]]).to_flat_index()
            flowveldepth = pd.concat([pd.DataFrame(r[1], index=r[0], columns=qvd_columns) for r in self.results], copy=False)


            if wbdyo and not self.waterbodies_df.empty:
                i_columns = pd.MultiIndex.from_product([range(self.nts), ["i"]]).to_flat_index()
                wbdy = pd.concat([pd.DataFrame(r[6], index=r[0], columns=i_columns) for r in self.results], copy=False)
                wbdy_id_list = self.waterbodies_df.index.values.tolist()
                flow_df = flowveldepth.loc[wbdy_id_list]
                wbdy = wbdy.loc[wbdy_id_list]
                
                if not self.duplicate_ids_df.empty:
                    flow_df = flow_df.rename(index=dict(self.duplicate_ids_df[['synthetic_ids','lake_id']].values))
                    wbdy = wbdy.rename(index=dict(self.duplicate_ids_df[['synthetic_ids','lake_id']].values))

                timestep, variable = zip(*flow_df.columns.tolist())
                timestep_index = np.where(((np.array(list(set(list(timestep)))) + 1) * self.dt) % (self.dt * self.qts_subdivisions) == 0)
            ...
            if self.link_lake_crosswalk:
                flowveldepth = self._reindex_lake_to_link_id(flowveldepth, self.link_lake_crosswalk)
            
            if self.return_courant:
                courant_columns = pd.MultiIndex.from_product([range(self.nts), ["cn", "ck", "X"]]).to_flat_index()
                courant = pd.concat([pd.DataFrame(r[2], index=r[0], columns=courant_columns) for r in self.results], copy=False)
                if self.link_lake_crosswalk:
                    courant.set_index(flowveldepth.index, inplace=True)

            LOG.debug("Constructing the FVD DataFrame took %s seconds." % (time.time() - start))
        
        if stream_output:
            self._handle_stream_output(stream_output, flowveldepth)
        
        if test:
            flowveldepth.to_pickle(Path(test))
        
        if wbdyo and not self.waterbodies_df.empty:
            self._handle_lakeout_output(wbdyo, i_df, q_df, d_df)

        if rsrto:
            self._handle_restart_output(rsrto, flowveldepth)
        
        if chrto:
            self._handle_chrtout_output(chrto, flowveldepth)
        
        if csv_output_folder:
            self._handle_csv_output(csv_output_folder, csv_output_segments, flowveldepth, courant=None)
        
        if parquet_output_folder:
            self._handle_parquet_output(parquet_output_folder, parquet_output_segments, flowveldepth, courant=None)
        
        if chano:
            self._handle_chanobs_output(chano, flowveldepth)
        
        if lastobso:
            self._handle_lastobs_output(lastobso)
    ...
    def perform_parity_check(self):
        if self.parity_set:
            ...
            start_time = time.time()
            parity_check(self.parity_set, self.results)
            LOG.debug("parity check complete in %s seconds." % (time.time() - start_time))

    def _handle_stream_output(self, stream_output, flowveldepth):
        ...
        nudge = np.concatenate([r[8] for r in self.results])
        usgs_positions_id = np.concatenate([r[3][0] for r in self.results])
        ...
        nhd_io.write_flowveldepth(
            ...
            poi_crosswalk=self.poi_crosswalk,
            nexus_dict=self.nexus_dict,
        )

        if self.logFileName != 'NONE':
            with open(self.logFileName, 'a') as preRunLog:
                preRunLog.write("\n") 
                preRunLog.write("-----\n") 
                ...
                fCalc = int(self.dt / 60)
                preRunLog.write(f"Internal computation of FVD data every {stream_output_internal_frequency} minutes\n")
                preRunLog.write(f"Output of FVD data every {fCalc} minutes\n")
                preRunLog.write(f"Writing {nTimeBins} time bins for {len(flowveldepth.index)} segments per FVD output file\n")
            preRunLog.close()

    def _handle_lakeout_output(self, wbdyo, i_df, q_df, d_df):
        time_index, tmp_variable = map(list, zip(*i_df.columns.tolist()))
        
        if not self.duplicate_ids_df.empty:
            output_waterbodies_df = self.waterbodies_df.rename(index=dict(self.duplicate_ids_df[['synthetic_ids', 'lake_id']].values))
            output_waterbody_types_df = self.waterbody_types_df.rename(index=dict(self.duplicate_ids_df[['synthetic_ids', 'lake_id']].values))
        else:
            output_waterbodies_df = self.waterbodies_df
            output_waterbody_types_df = self.waterbody_types_df
        ...
        if self.logFileName != 'NONE':
            with open(self.logFileName, 'a') as preRunLog:
                preRunLog.write("\n")
                preRunLog.write("-----\n")
                preRunLog.write(f"Output of waterbody files into folder: {wbdyo}\n")
                preRunLog.write("-----\n")
            preRunLog.close()
    ...
    def _handle_restart_output(self, rsrto, flowveldepth):
        LOG.info("- writing restart files")
        start = time.time()

        wrf_hydro_restart_dir = rsrto.get("wrf_hydro_channel_restart_source_directory", None)

        if wrf_hydro_restart_dir:
            restart_pattern_filter = rsrto.get("wrf_hydro_channel_restart_pattern_filter", "HYDRO_RST.*")
            wrf_hydro_restart_files = sorted(Path(wrf_hydro_restart_dir).glob(restart_pattern_filter))

            if wrf_hydro_restart_files:
                nhd_io.write_hydro_rst(
                    flowveldepth,
                    wrf_hydro_restart_files,
                    self.restart_parameters.get("wrf_hydro_channel_restart_file"),
                    self.dt,
                    self.nts,
                    self.t0,
                    self.restart_parameters.get("wrf_hydro_channel_ID_crosswalk_file"),
                    self.restart_parameters.get("wrf_hydro_channel_ID_crosswalk_file_field_name"),
                )
                ...
                if self.logFileName != 'NONE':
                    with open(self.logFileName, 'a') as preRunLog:
                        preRunLog.write("\n")
                        preRunLog.write("-----\n")
                        preRunLog.write(f"Output of wrf hydro restart files into folder: {Path(wrf_hydro_restart_dir)}\n")
                        preRunLog.write("-----\n")
                    preRunLog.close()
    ...
    def _handle_chrtout_output(self, chrto, flowveldepth):
        ...
        if chrtout_read_folder:
            chrtout_files = sorted(Path(chrtout_read_folder) / f for f in self.run["qlat_files"])
            ...
            if self.logFileName != 'NONE':
                with open(self.logFileName, 'a') as preRunLog:
                    preRunLog.write("\n")
                    preRunLog.write("-----\n")
                    preRunLog.write(f"Output of FVD files into folder: {chrtout_read_folder}\n")
                    preRunLog.write("-----\n")
                preRunLog.close()

        LOG.debug("writing CHRTOUT files took a total time of %s seconds." % (time.time() - start))

    def _handle_csv_output(self, csv_output_folder, csv_output_segments, flowveldepth, courant):
        LOG.info("- writing flow, velocity, and depth results to .csv")
        start = time.time()

        # Create filenames
        if self.supernetwork_parameters.get("title_string", None):
            filename_fvd = f"flowveldepth_{self.supernetwork_parameters['title_string']}.csv"
            filename_courant = f"courant_{self.supernetwork_parameters['title_string']}.csv"
        else:
            run_time_stamp = datetime.now().isoformat()
            filename_fvd = f"flowveldepth_{run_time_stamp}.csv"
            filename_courant = f"courant_{run_time_stamp}.csv"
    ...
    def _handle_parquet_output(self, parquet_output_folder, parquet_output_segments, flowveldepth, courant):
        LOG.info("- writing flow, velocity, and depth results to .parquet")
        start = time.time()

        # Create filenames
        if self.supernetwork_parameters.get("title_string", None):
            filename_fvd = f"flowveldepth_{self.supernetwork_parameters['title_string']}.parquet"
            filename_courant = f"courant_{self.supernetwork_parameters['title_string']}.parquet"
        else:
            run_time_stamp = datetime.now().isoformat()
            filename_fvd = f"flowveldepth_{run_time_stamp}.parquet"
            filename_courant = f"courant_{run_time_stamp}.parquet"
        ...
        flowveldepth = flowveldepth.sort_index()
        configuration = self.output_parameters["parquet_output"].get("configuration")
        prefix_ids = self.output_parameters["parquet_output"].get("prefix_ids")
        
        timeseries_df = self._parquet_output_format_converter(
            flowveldepth,
            self.restart_parameters.get("start_datetime"),
            self.dt,
            configuration,
            prefix_ids
        )

        parquet_output_segments_str = [f"{prefix_ids}-{segment}" for segment in parquet_output_segments]
        ...
        if self.return_courant:
            courant = courant.sort_index()
            timeseries_courant = self._parquet_output_format_converter(
                courant,
                self.restart_parameters.get("start_datetime"),
                self.dt,
                configuration,
    ...
    def _handle_chanobs_output(self, chano, flowveldepth):
        LOG.info("- writing t-route flow results at gage locations to CHANOBS file")
        start = time.time()

        if self.link_lake_crosswalk:
            self.link_gage_df = self._reindex_lake_to_link_id(self.link_gage_df, self.link_lake_crosswalk)

        chanobs_output_directory = chano['chanobs_output_directory']
        if not isinstance(chanobs_output_directory, str):
            chanobs_output_directory = str(chanobs_output_directory) + '/'
        chanobs_filepath = str(chano['chanobs_filepath'])

        nhd_io.write_chanobs(
            Path(chanobs_output_directory + chanobs_filepath),
            flowveldepth,
            self.link_gage_df,
            self.t0,
            self.dt,
        ...
        if self.logFileName != 'NONE':
            with open(self.logFileName, 'a') as preRunLog:
                preRunLog.write("\n")
                preRunLog.write("-----\n")
                preRunLog.write(f"Output of results at gage locations into folder: {chanobs_output_directory}\n")
                preRunLog.write("-----\n")
            preRunLog.close()

        LOG.debug("writing flow data to CHANOBS took %s seconds." % (time.time() - start))

    def _handle_lastobs_output(self, lastobso):
        ...
        streamflow_da = self.data_assimilation_parameters.get('streamflow_da', None)
        if streamflow_da:
            lastobs_output_folder = self.output_parameters.get("lastobs_output", None)
            nudging_true = streamflow_da.get('streamflow_nudging', None)

        if nudging_true and lastobs_output_folder and not self.lastobs_df.empty:
            ...
            if self.logFileName != 'NONE':
                with open(self.logFileName, 'a') as preRunLog:
                    preRunLog.write("\n")
                    preRunLog.write("-----\n")
                    preRunLog.write(f"Output of lastobs data into folder: {lastobs_output_folder}\n")
                    preRunLog.write("-----\n")
                preRunLog.close()
