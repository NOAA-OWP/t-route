def print_basic_network_info(
    ...
    for key, value in connections.items():
        try:
            ...
            if debuglevel <= -2 and len(value["upstreams"]) > 1:
                print(
                    f"junction# {i} at key {key} has these upstream segments {value['upstreams']}"
                )
        except:
            if debuglevel <= -1:
                print(
                    f"No valid upstream computed for segment {key} -- possible circular reference"
                )
            if debuglevel <= -2:
                print(
                    f" key {key} points to {value['downstream']} which points to {connections[value['downstream']]['downstream']}"
                )
    ...
    if junction_keys:
        ...
        print(
            f"Total Reaches ( = head_segments + junction_nodes ) {len(junction_keys) + len(headwater_keys)}"
        )
    if terminal_keys:
        print(
            f"Total Independent Networks (estimated from number of terminal keys) {len(terminal_keys)}"
        )
...
def rec_print_down(key, down_connections, terminal_ref_keys, debuglevel=0):
    if key in terminal_ref_keys:
        return
    print(f"{key} with length {down_connections[key]['length']}")
    rec_print_down(
        down_connections[key]["downstream"],
        down_connections,
        terminal_ref_keys,
        debuglevel,
    )
...
def rec_print_up(
    keys, tab_count, up_connections, down_connections, terminal_code, debuglevel=0
):
    ...
    for key in keys:
        if not key == terminal_code:
            ...
            print(
                f"{'..' * (tab_count)}\\{key} with length {down_connections[key]['length']}\\"
            )
            rec_print_up(
...
def print_connections(
    ...
    try:
        ...
        if debuglevel <= -1:
            print(
                """Error: Provide headwater_keys, down_connections, and a terminal code
