...
def prune_headwaters(data, threshold, network_data):
    ...
    while 1 == 1:
        ...
        connections, rconn = network_connections(data.drop(index = chain.from_iterable(hw_prune_list)), network_data)
            ...
            # two short headwaters draining to the same junction
                ...
                for y, (tw_y, jun_y) in enumerate(hw_junctions.items()):     
                    ...
                    # the correlary headwater reach draining to the same junction has been found
                        ...
                        if data.loc[reach1,"Length"].sum() <= data.loc[reach2,"Length"].sum():
                            # trim reach1
...
def snap_junctions(data, threshold, network_data):
    ...
    while 1 == 1:
        ...
        if iter_num > 1:
            ...
            print("Prior to itteration", iter_num, ",", len(short_reaches), "short reaches exist in the network")
...
def update_network_data(data, rch, data_merged, chop, rconn):
    ...
    if bool(upstreams):
        ...
        data.loc[upstreams, "to"] = data_merged.head(1).index.values[0]  # index of NEW reach head
